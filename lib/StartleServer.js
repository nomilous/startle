...
module.exports = class StartleServer extends EventEmitter {
  ...
  _doProcessStart(socket, msg) {
    ...
    child.on('message', message => {
      ...
      switch (message.action) {
        case 'metrics':
          this._updateMetrics(socket, message.metrics);
          break;
  ...
  _updateMetrics(socket, newMetrics) {
    const metrics = socket.metrics;

    for (var name in newMetrics.counters) {
      metrics.counters[name] = metrics.counters[name] || 0;
      metrics.counters[name] += newMetrics.counters[name];
    }
